...
- [ ] 测试在模板缺失时失败。
- [ ] 敏感信息断言覆盖 token/key/pem/env 等关键词。
...
- [ ] 内容不同且未 force 的测试期望 exit code 1 或等价错误对象。
...
#### 验收标准
- [ ] 全局目标为 `<home>/.claude/skills/doc-sync/SKILL.md`。
- [ ] 项目目标为 `<cwd>/.claude/skills/doc-sync/SKILL.md`。
- [ ] install 不默认覆盖不同内容。
- [ ] update 与 `install --force` 可覆盖不同内容。
- [ ] `--backup` 在覆盖前生成备份。
...
- [ ] README 中补充 `docsync skill` 用法。
- [ ] AGENTS 中补充 Skill 管理验证命令。
- [ ] CHANGELOG 中记录 Claude Skill 管理能力。
